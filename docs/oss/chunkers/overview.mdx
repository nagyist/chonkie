...
All chunkers support chunk overlap natively via the `chunk_overlap` parameter. This adds overlapping context between adjacent chunks, which improves retrieval quality in RAG pipelines.

```python
from chonkie import TokenChunker, SentenceChunker, RecursiveChunker

# Any chunker accepts chunk_overlap
...
For advanced overlap configuration (recursive mode, custom rules), see the [Overlap Refinery](/oss/refinery/overlap-refinery) documentation.
