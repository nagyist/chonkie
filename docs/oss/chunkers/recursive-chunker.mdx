...
<ParamField path="chunk_overlap / chunkOverlap" type="int" default="0">
  Number of overlapping tokens between chunks. See [Overlap Refinery](/oss/refinery/overlap-refinery) for advanced overlap options (`overlap_mode`, `overlap_method`, `overlap_rules`).
</ParamField>
